...
- For each new or updated visualization, choose a concise ASCII
  ...
  project's `.craft/visualizations/` directory. Create that directory when it
  does not exist. Use the directive path
  `.craft/visualizations/<title>.html`.
...
- Check that JavaScript has no undefined identifiers, every queried element
  ...
  `python3 <skill-directory>/scripts/render.py <absolute-fragment-path> [<destination>.html] [--serve]`,
  where `<skill-directory>` is the directory containing this `SKILL.md`.
...
```text
::codex-inline-vis{file=".craft/visualizations/<title>.html"}
```
...
- Keep the fragment as the editable inline source. When the user explicitly asks
  ...
  `python3 <skill-directory>/scripts/render.py <absolute-fragment-path> <destination>.html`.
- If the visualization calls `window.openai`, replace that host-only interaction
  before using the standalone HTML outside Craft Agent.
