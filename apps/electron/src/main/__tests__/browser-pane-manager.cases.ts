...
const mockMenuPopup = mock(() => {})
const mockMenuBuild = mock((_template: unknown[]) => ({ popup: mockMenuPopup }))
...
function createMockWebContentsView() {
  ...
  return {
    webContents,
    setBounds: mock((nextBounds: typeof bounds) => { bounds = nextBounds }),
    getBounds: mock(() => bounds),
    setVisible: mock((nextVisible: boolean) => { visible = nextVisible }),
    getVisible: mock(() => visible),
  ...
  const win = {
    ...
    contentView: {
      ...
      addChildView: mock((view: any) => {
        const existing = childViews.indexOf(view)
        if (existing >= 0) childViews.splice(existing, 1)
    ...
    setBounds: mock((bounds: { width?: number; height?: number }) => {
      if (bounds.width !== undefined) contentWidth = bounds.width
      if (bounds.height !== undefined) contentHeight = bounds.height
    }),
...
describe('BrowserPaneManager', () => {
  ...
  it('embeds a manual runtime and lets a local agent reuse the same instance', () => {
    const hostWindow = createMockWindow({ width: 1400, height: 900 })
    ;(hostWindow as any).getContentBounds = mock(() => ({ x: 100, y: 80, width: 1400, height: 900 }))
    manager.setWindowManager({
      getWindowByWebContentsId: mock((id: number) => id === 42 ? hostWindow : null),
    } as any)

    const id = (manager as any).createInstance('embedded-1', {
      workspaceId: 'workspace-1',
      embeddedHostWebContentsId: 42,
    })
    const instance = (manager as any).instances.get(id)

    expect(instance.window._options.hasShadow).toBe(false)

    manager.setEmbeddedBounds(id, 42, { x: 320, y: 120, width: 800, height: 600 })
    manager.setEmbeddedVisible(id, 42, true)

    expect(instance.pageView.setBounds).toHaveBeenLastCalledWith({ x: 320, y: 120, width: 800, height: 600 })
    expect(instance.pageView.setBorderRadius).toHaveBeenLastCalledWith(10)
    ...
    expect(hostWindow.contentView._children).toContain(instance.pageView)

    const reused = manager.createForSession('session-1', { workspaceId: 'workspace-1' })
  ...
  it('shows toolbar menus above an embedded page by parenting them to the visible host', () => {
    ...
    ;(manager as any).createInstance('embedded-menu', { embeddedHostWebContentsId: 42 })

    manager.showToolbarMenu('extensions', 'embedded-menu')
  ...
  it('reveals the native floating toolbar without moving the embedded page', () => {
    ...
    ;(manager as any).createInstance('floating-toolbar', { embeddedHostWebContentsId: 42 })
    manager.setEmbeddedToolbarMode('floating-toolbar', 42, 'floating')

    manager.setEmbeddedBounds('floating-toolbar', 42, { x: 320, y: 120, width: 800, height: 600 })
    manager.setEmbeddedVisible('floating-toolbar', 42, true)
    const instance = (manager as any).instances.get('floating-toolbar')
    const pageBounds = instance.pageView.getBounds()

    const pageSetBoundsCallCount = instance.pageView.setBounds.mock.calls.length
    const childReorderCallCount = hostWindow.contentView.removeChildView.mock.calls.length
    manager.setEmbeddedBounds('floating-toolbar', 42, { x: 320, y: 120, width: 800, height: 600 })
    expect(instance.pageView.setBounds.mock.calls).toHaveLength(pageSetBoundsCallCount)
    expect(hostWindow.contentView.removeChildView.mock.calls).toHaveLength(childReorderCallCount)

    expect(instance.toolbarView.getBounds()).toEqual({ x: 328, y: 128, width: 784, height: 8 })
    expect(instance.toolbarView.setBorderRadius).toHaveBeenLastCalledWith(0)
    instance.toolbarView.webContents._emit('before-input-event', { type: 'mouseMove' })

    expect(instance.embeddedToolbarRevealed).toBe(true)
    expect(instance.toolbarView.getBounds()).toEqual({ x: 328, y: 128, width: 784, height: 48 })
    expect(instance.toolbarView.setBorderRadius).toHaveBeenLastCalledWith(0)
    expect(instance.toolbarView.getVisible()).toBe(true)
    expect(instance.pageView.getBounds()).toEqual(pageBounds)

    manager.registerToolbarIpc()
    const revealRegistration = (
      mockIpcMainHandle.mock.calls as unknown as Array<[
        string,
        (_event: unknown, instanceId: string, revealed: boolean) => void,
      ]>
    ).find(([channel]) => channel === 'browser-toolbar:set-revealed')
    expect(revealRegistration).toBeTruthy()
    if (!revealRegistration) throw new Error('Expected browser-toolbar:set-revealed IPC registration')

    const [, revealHandler] = revealRegistration
    const setBoundsCallCount = instance.toolbarView.setBounds.mock.calls.length
    const removeChildCallCount = hostWindow.contentView.removeChildView.mock.calls.length
    revealHandler({}, 'floating-toolbar', true)
    revealHandler({}, 'floating-toolbar', true)

    expect(instance.toolbarView.setBounds.mock.calls).toHaveLength(setBoundsCallCount)
    expect(hostWindow.contentView.removeChildView.mock.calls).toHaveLength(removeChildCallCount)

    manager.setEmbeddedToolbarMode('floating-toolbar', 42, 'fixed')
    expect(instance.toolbarView.setVisible).toHaveBeenLastCalledWith(false)
    expect(manager.listInstances()[0].toolbarMode).toBe('fixed')
  })

  it('pins a floating toolbar through IPC and emits the fixed state', () => {
    const hostWindow = createMockWindow({ width: 1400, height: 900 })
    ;(hostWindow as any).getContentBounds = mock(() => ({ x: 0, y: 0, width: 1400, height: 900 }))
    manager.setWindowManager({
      getWindowByWebContentsId: mock((id: number) => id === 42 ? hostWindow : null),
    } as any)
    ;(manager as any).createInstance('pin-toolbar', { embeddedHostWebContentsId: 42 })
    manager.setEmbeddedToolbarMode('pin-toolbar', 42, 'floating')
    manager.setEmbeddedBounds('pin-toolbar', 42, { x: 100, y: 100, width: 900, height: 600 })
    manager.setEmbeddedVisible('pin-toolbar', 42, true)
    ...
    const pinRegistration = (
      mockIpcMainHandle.mock.calls as unknown as Array<[
        string,
        (_event: unknown, instanceId: string) => boolean,
      ]>
    ).find(([channel]) => channel === 'browser-toolbar:pin-embedded')
    expect(pinRegistration).toBeTruthy()
    if (!pinRegistration) throw new Error('Expected browser-toolbar:pin-embedded IPC registration')

    const instance = (manager as any).instances.get('pin-toolbar')
    const [, pinHandler] = pinRegistration
    const pinned = pinHandler({ sender: instance.toolbarView.webContents }, 'stale-toolbar-id')

    expect(pinned).toBe(true)
    expect(manager.listInstances()[0].toolbarMode).toBe('fixed')
    expect(states.at(-1)?.toolbarMode).toBe('fixed')
    expect(instance.toolbarView.setVisible).toHaveBeenLastCalledWith(false)

    manager.setEmbeddedToolbarMode('pin-toolbar', 42, 'floating')
    expect(manager.listInstances()[0].toolbarMode).toBe('floating')
    expect(states.at(-1)?.toolbarMode).toBe('floating')

    const pinnedAgain = pinHandler({ sender: instance.toolbarView.webContents }, '')
    expect(pinnedAgain).toBe(true)
    expect(manager.listInstances()[0].toolbarMode).toBe('fixed')
    expect(states.at(-1)?.toolbarMode).toBe('fixed')
  })

  it('uses the same 10px radius for the native page and agent overlay', () => {
    manager.createInstance('uniform-radius')
    const instance = (manager as any).instances.get('uniform-radius')
    const overlayUrl = instance.nativeOverlayView.webContents.loadURL.mock.calls[0][0]

    expect(decodeURIComponent(overlayUrl)).toContain('border-radius: 10px')
  })

  it('extracts ZIP payloads from imported CRX3 extension packages', () => {
    const zipPayload = Buffer.from([0x50, 0x4b, 0x03, 0x04, 0x14, 0x00])
    const crxHeader = Buffer.alloc(12)
    crxHeader.write('Cr24', 0, 'ascii')
    crxHeader.writeUInt32LE(3, 4)
    crxHeader.writeUInt32LE(0, 8)

    const extracted = (manager as any).extractCrxZipPayload(Buffer.concat([crxHeader, zipPayload]))
    expect(extracted).toEqual(zipPayload)
  })

  it('moves a visible standalone runtime into the host without leaving it hidden', () => {
    const hostWindow = createMockWindow({ width: 1400, height: 900 })
    ;(hostWindow as any).getContentBounds = mock(() => ({ x: 0, y: 0, width: 1400, height: 900 }))
    manager.setWindowManager({
      getWindowByWebContentsId: mock((id: number) => id === 42 ? hostWindow : null),
    ...
    manager.createInstance('standalone-to-embedded')
    const instance = (manager as any).instances.get('standalone-to-embedded')
    instance.toolbarReady = true
    manager.focus('standalone-to-embedded')

    manager.setEmbeddedBounds('standalone-to-embedded', 42, { x: 320, y: 120, width: 800, height: 600 })
    manager.setEmbeddedVisible('standalone-to-embedded', 42, true)

    expect(instance.window.contentView._children).not.toContain(instance.pageView)
    expect(hostWindow.contentView._children).toContain(instance.pageView)
    expect(instance.pageView.setVisible).toHaveBeenLastCalledWith(true)
    expect(manager.listInstances().find((item) => item.id === 'standalone-to-embedded')?.isVisible).toBe(true)
  })
  ...
  it('destroys child popups when parent instance is destroyed', () => {
    ...
    instance.pageView.webContents._listeners['did-create-window'][0](
      popupWindow,
  ...
  it('navigates from the address-only new-tab page', async () => {
    manager.createInstance('nav-empty-state')
    const instance = (manager as any).instances.get('nav-empty-state')
    const emptyStateUrl = 'file:///browser-empty-state.html#navigate=value=example.com&ts=1'
    instance.pageView.webContents.getURL = mock(() => emptyStateUrl)

    instance.pageView.webContents._emit('did-navigate-in-page', emptyStateUrl)
    await Bun.sleep(0)
  ...
  it('focus brings the instance window to front', () => {
    ...
    instance.toolbarView.webContents.getURL = mock(() => 'http://localhost/browser-toolbar.html')
