...
export function SessionItem({
  ...
  return (
    ...
    <EntityRow
      ...
      icon={
        <>
          ...
          <div className={cn(
            ...
            (item.isProcessing || item.isPinned || hasUnreadMeta(item) || item.lastMessageRole === 'plan' || hasPendingPrompt)
              ? "opacity-100 ml-0"
              : "!w-0 opacity-0 -ml-[10px]"
          )}>
            {item.isProcessing && <Spinner className="text-[10px]" />}
            {item.isPinned && <Pin className="h-3 w-3 text-muted-foreground" aria-label="Pinned" />}
            {hasUnreadMeta(item) && (
      ...
      titleClassName={cn("text-control", item.isAsyncOperationOngoing && "animate-shimmer-text")}
      ...
      titleTrailing={hasMatch ? (
        <span
          className={cn(
            "inline-flex items-center justify-center min-w-[24px] px-1 py-0.5 rounded-control text-[10px] font-medium tabular-nums leading-tight whitespace-nowrap shadow-tinted",
            isSelected
