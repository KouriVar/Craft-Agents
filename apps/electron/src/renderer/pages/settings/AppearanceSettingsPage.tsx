...
export interface AppearanceSettingsPageProps {
  embedded?: boolean
  showDefaultTheme?: boolean
  showWorkspaceThemes?: boolean
  showInterface?: boolean
  showKanban?: boolean
  /** Column status mapping on drag — behavior, not appearance */
  showKanbanBehavior?: boolean
...
export default function AppearanceSettingsPage({
  embedded = false,
  showDefaultTheme = true,
  showWorkspaceThemes = true,
  showInterface = true,
  showKanban = true,
  showKanbanBehavior = true,
  showToolIcons = true,
  ...
  const handleWindowsBackgroundEffectChange = useCallback((checked: boolean) => {
    setWindowsBackgroundEffectEnabledState(checked)
    storage.set(storage.KEYS.windowsBackgroundEffectEnabled, checked)
    window.dispatchEvent(new CustomEvent('craft:windows-background-effect-changed', { detail: checked }))
  }, [])

  const [shellGlassIntensity, setShellGlassIntensityState] = useState<GlassIntensity>(() =>
    storage.get(storage.KEYS.shellGlassIntensity, 'medium') as GlassIntensity,
  )
  const handleShellGlassIntensityChange = useCallback((value: string) => {
    const next = value as GlassIntensity
    setShellGlassIntensityState(next)
    storage.set(storage.KEYS.shellGlassIntensity, next)
    window.dispatchEvent(new CustomEvent('craft:shell-glass-intensity-changed', { detail: next }))
  }, [])

  const [panelGlassIntensity, setPanelGlassIntensityState] = useState<GlassIntensity>(() =>
    storage.get(storage.KEYS.panelGlassIntensity, 'off') as GlassIntensity,
  )
  const handlePanelGlassIntensityChange = useCallback((value: string) => {
    const next = value as GlassIntensity
    setPanelGlassIntensityState(next)
    storage.set(storage.KEYS.panelGlassIntensity, next)
    window.dispatchEvent(new CustomEvent('craft:panel-glass-intensity-changed', { detail: next }))
  }, [])
  ...
  const body = (
    <div className="space-y-8">
      ...
      {showWorkspaceThemes && workspaces.length > 0 && (
        <SettingsSection
      ...
      {showInterface && (
        <SettingsSection title={t('settings.appearance.interface')}>
          <SettingsCard>
            ...
            {isWindows ? (
              <SettingsToggle
                label={t('settings.appearance.windowsBackgroundEffect', { defaultValue: '背景效果' })}
                description={t('settings.appearance.windowsBackgroundEffectDesc', { defaultValue: '统一控制窗口背景、顶部导航栏和侧栏的透明与模糊效果。' })}
                checked={windowsBackgroundEffectEnabled}
                onCheckedChange={handleWindowsBackgroundEffectChange}
              />
            ) : (
              <>
                <SettingsRow
                  label={t('settings.appearance.shellGlassIntensity', { defaultValue: '外层玻璃效果' })}
                  description={t('settings.appearance.shellGlassIntensityDesc', { defaultValue: '只调整窗口最外层背景，依赖系统原生玻璃材质。' })}
                >
                  <SettingsMenuSelect
                    value={shellGlassIntensity}
                    onValueChange={handleShellGlassIntensityChange}
                    options={[
                      { value: 'off', label: t('settings.appearance.shellGlassOff', { defaultValue: '关闭' }) },
                      { value: 'subtle', label: t('settings.appearance.shellGlassSubtle', { defaultValue: '轻微' }) },
                      { value: 'medium', label: t('settings.appearance.shellGlassMedium', { defaultValue: '中等' }) },
                      { value: 'strong', label: t('settings.appearance.shellGlassStrong', { defaultValue: '明显' }) },
                      { value: 'max', label: t('settings.appearance.shellGlassMax', { defaultValue: '强烈' }) },
                    ]}
                  />
                </SettingsRow>
                <SettingsRow
                  label={t('settings.appearance.panelGlassIntensity', { defaultValue: '卡片玻璃效果' })}
                  description={t('settings.appearance.panelGlassIntensityDesc', { defaultValue: '调整左侧导航、列表、主内容和右侧信息面板的透明度。' })}
                >
                  <SettingsMenuSelect
                    value={panelGlassIntensity}
                    onValueChange={handlePanelGlassIntensityChange}
                    options={[
                      { value: 'off', label: t('settings.appearance.panelGlassOff', { defaultValue: '关闭' }) },
                      { value: 'subtle', label: t('settings.appearance.panelGlassSubtle', { defaultValue: '轻微' }) },
                      { value: 'medium', label: t('settings.appearance.panelGlassMedium', { defaultValue: '中等' }) },
                      { value: 'strong', label: t('settings.appearance.panelGlassStrong', { defaultValue: '明显' }) },
                      { value: 'max', label: t('settings.appearance.panelGlassMax', { defaultValue: '强烈' }) },
                    ]}
                  />
                </SettingsRow>
              </>
      ...
      {showKanbanBehavior && (
        <SettingsSection
          ...
          description={t('settings.interface.kanbanBehaviorDesc')}
        >
          <SettingsCard>
            {KANBAN_COLUMNS.map(column => (
              <SettingsRow key={column.id} label={t(column.labelKey)}>
      ...
      {showToolIcons && (
        <details className="group space-y-3">
          <summary className="cursor-pointer list-none pl-1 text-base font-semibold marker:content-none [&::-webkit-details-marker]:hidden">
            <span className="inline-flex items-center gap-2">
              {t('settings.interface.advancedUi')}
              <span className="text-xs font-normal text-muted-foreground">
                ({t('settings.appearance.toolIcons')})
              </span>
            </span>
          </summary>
          <SettingsSection
