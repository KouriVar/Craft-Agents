...
/**
 * Result of consuming a pairing code: bind the originating session to the chat
 * the `/pair` command was typed in.
 */
...
/**
 * Some platforms let users address commands to specific bots in shared chats:
 * `/pair@MyBot 123456`. Without stripping the `@BotName` suffix, the cmd token
 * doesn't match our switch cases for users typing the canonical group form.
 *
...
export class Commands {
  ...
  async handleCommand(adapter: PlatformAdapter, msg: IncomingMessage): Promise<boolean> {
    ...
    // Strip the optional `@BotName` suffix used to disambiguate commands in
    // shared chats. Without this, `/pair@MyBot 123456` would never match the
    // switch case below.
  ...
  private async handleHelp(adapter: PlatformAdapter, msg: IncomingMessage): Promise<void> {
    const bindLine = '/bind — pick from recent sessions\n'
    const replyOpts = msg.threadId !== undefined ? { threadId: msg.threadId } : {}
